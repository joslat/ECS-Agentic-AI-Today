...
public static class D04_AgenticCollaboration
{
    public static async Task Execute()
    {
        ...
        var critic = CreateCriticAgent(kernel);
...
public sealed class ApprovalTerminationStrategy : TerminationStrategy
{
    ...
    protected override Task<bool> ShouldAgentTerminateAsync(Agent agent, IReadOnlyList<ChatMessageContent> history, CancellationToken cancellationToken)
        => Task.FromResult(history[history.Count - 1].Content?.Equals("approve", StringComparison.OrdinalIgnoreCase) ?? false);
}
